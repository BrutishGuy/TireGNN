...
class AbstractAE(nn.Module):
    ...
    def encode_windows(self, windows):
        ...
        print(self.prepare_input_windows(windows).shape)
        with torch.no_grad():
...
class SeqAE(AbstractAE):
    def __init__(self,
        ...
        self.encoder_shared = nn.Sequential(
            nn.Linear(self.num_layers * self.embedding_dim, nr_shared),
            nn.Tanh()
        )
        self.encoder_unshared = nn.Sequential(
            nn.Linear(self.num_layers * self.embedding_dim, latent_dim-nr_shared),
            nn.Tanh()
        )
        ...
        #self.decoder_seq_layer = nn.LSTM(
    ...
    def encode(self, x):
        ...
        x = x.reshape((batch_size * self.nr_ae, self.window_size, self.input_dim))
        #print(x.shape)
        z, (hidden_z, _) = self.encoder(x)
        #print(hidden_z.shape)
        hidden_z = hidden_z.reshape((batch_size, self.nr_ae, self.num_layers * self.embedding_dim))
...
class SequentialTIRE(AbstractTIRE):
    def __init__(self, input_dim:int, window_size:int=20,
        ...
        self.AE_TD = SeqAE(input_dim, num_input_channels=input_dim, window_size=self.window_size_td, intermediate_dim=intermediate_dim_TD, **kwargs)
        self.AE_FD = SeqAE(input_dim, num_input_channels=input_dim, window_size=self.window_size_fd, intermediate_dim=intermediate_dim_FD, **kwargs)