...
from SeqTIRE import utils

# The code is taken from https://github.com/deryckt/TIRE/blob/master/simulate.py

def generate_multichannel_simulated_dataset(generating_func, func_params, num_channels):
    
    channel_windows, channel_timeseries, channel_parameters = [], [], []
    
    for chan in range(num_channels):
        channel_window_set, channel_series, channel_parameter_set = generating_func(**func_params)
        channel_windows.append(channel_window_set)
        channel_timeseries.append(channel_series)
        channel_parameter.append(channe_parameter_set)
     
    channel_windows, channel_timeseries, channel_parameters = np.array(channel_windows), np.array(channel_timeseries), np.array(channel_parameters)
    
    channel_windows = np.reshape(channel_windows)
    channel_timeseries = np.reshape(channel_timeseries)
    channel_parameters = np.reshape(channel_parameters)
    
    return channel_timeseries, channel_windows, channel_parameters
...
def generate_changingcoefficients(window_size, stride=1, nr_cp=49, delta_t_cp = 1000, delta_t_cp_std = 10, scale_min=-1, scale_max=1):
    ...
    parameters = utils.ts_to_windows(parameters,0,1,stride)
