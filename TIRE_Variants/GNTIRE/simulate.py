...
def generate_multichannel_simulated_dataset(generating_func, func_params, num_channels):
    ...
    for chan in range(num_channels):
        channel_series, channel_window_set, channel_parameter_set = generating_func(**func_params)
        channel_windows.append(channel_window_set)
        channel_timeseries.append(channel_series)
        channel_parameters.append(channel_parameter_set)
     
    channel_windows, channel_timeseries, channel_parameters = stack_padding(channel_windows), stack_padding(channel_timeseries), stack_padding(channel_parameters)
    
    channel_windows = np.transpose(channel_windows, (1, 0))
    channel_timeseries = np.transpose(channel_timeseries, (1, 0))
    channel_parameters = np.transpose(channel_parameters, (1, 0))
